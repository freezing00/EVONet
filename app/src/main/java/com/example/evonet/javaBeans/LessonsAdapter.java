...
public class LessonsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView==null)
        {
            ...
            convertView=LayoutInflater.from(context).inflate(R.layout.list_teach_item,parent,false);
            viewHolder.textView_lesson_name=convertView.findViewById(R.id.lesson_name_teah);
            viewHolder.textView_lesson_number=convertView.findViewById(R.id.lesson_number_teah);
            viewHolder.textView_lesson_sum_person=convertView.findViewById(R.id.lesson_sum_person_teah);
            viewHolder.textView_lesson_number.setTag(position);

            convertView.setTag(viewHolder);
        }else {
            viewHolder=(LessonsAdapter.ViewHolder)convertView.getTag();//主要是绑定布局文件中的id，不用每点击一次就去寻找，这里相当于一个容器
//            viewHolder.textView_lesson_number.setTag(position);
        }
    ...
    //	声明接口
    private LessonsAdapter.onItemDeleteListener xmOnItemDeleteListener;


    //	根据我们在回调里写的是 xmOnItemDeleteListener 调用的onDeleteClick
    //	系统就会执行下面的方法
    public void setXmOnItemDeleteListener(LessonsAdapter.onItemDeleteListener xmOnItemDeleteListener){
